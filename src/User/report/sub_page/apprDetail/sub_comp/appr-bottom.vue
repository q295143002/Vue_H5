<template>
  <div class="gx-footer">
  <div class="" v-if="bodyData.ApprovalStatus===0&&bodyData.type!='toOther'&&b(bodyData.EffectiveTimeSpan)">
    ...
    <x-dialog :show.sync="show1" class="dialog-demo">
      ...
      <div class="img-box">
        ...
        <flexbox  :gutter="0" class="btn-group">
          ...
          <flexbox-item >
            <x-button type="primary" class="agree font-8" @click="agreeAppr"><span style="visibility: hidden;">并</span>通 <span style="visibility: hidden;">转</span> 过<span style="visibility: hidden;">审</span></x-button>
          </flexbox-item>

          <flexbox-item :span="6/12" v-show="false" >
            <x-button type="primary" class="agree-transform font-8"  @click="agreeTrans">通过并转审</x-button>
          </flexbox-item>
        </flexbox>
      </div>
      <p class="gfd-hint" v-if="false">注：审批提交前请确认是否需要转交上一级审批</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      disagreeAppr () {
        // this.showX = true
        this.$http.get(window.Miu.Data.ApiURL + '/Approval_1_0/GetRejectReason', {
      ...
      agreeAppr () {
        ...
        this.isOperate(function () {
          _self.$http.post(window.Miu.Data.ApiURL + '/Approval_1_0/ApprovedApproval', {
            authTkn: window.Miu.Data.Token,
      ...
      reject () {
        ...
        _self.isOperate(function () {
          _self.$http.post(window.Miu.Data.ApiURL + '/Approval_1_0/TurnDownApproval', {
            authTkn: window.Miu.Data.Token,
            uid: _self.bottomData.uid,
            approvalId: _self.bottomData.approvalId,
            remark: _self.arText,

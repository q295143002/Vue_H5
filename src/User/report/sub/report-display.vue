<template>
 <div id="creport" style="background-color: #ddd;">
   ...
   <div class="scroller-wrap" v-bind:class="{ 'visible': !isToMe}" >
     <flexbox class="tc"  v-show="toMe1">
       <flexbox-item> 没有审批单</flexbox-item>
     </flexbox>
   <x-scroller  v-show="!toMe1" lock-x scrollbar-y use-pullup use-pulldown  @pullup:loading="loadMore" @pulldown:loading="refresh" v-ref:scroller>
     <div  v-bind:style="{'min-height':toMCount}">
      <div class="x_report">
          ...
          <cell @click="$router.go('/report/toMe/flight/'+items.ApprovalID)"><!--假数据传参-->
            <flexbox class="flx1">
              ...
              <flexbox-item :span="3" class="tl" >
                <div v-if="b(items.EffectiveTimeSpan)||items.ApprovalStatus!=0">
                ...
                <div class="x_icon_word">
                  <flexbox :gutter="0" style="margin: 5px 0;">
                    ...
                    <flexbox-item :span="5" class='tl'>
                      <span class=" x_f_small" v-if="items.ApprovalStatus===0&&b(items.EffectiveTimeSpan)">剩余审批时间{{items.ApprovalDate|timeLeft items.EffectiveTimeSpan}}分钟</span>
                    </flexbox-item>
   ...
   <div class="scroller-wrap" v-bind:class="{ 'visible': isToMe}">
     ...
     <x-scroller  v-show="!toOther1" lock-x scrollbar-y use-pullup use-pulldown  @pullup:loading="loadMore" @pulldown:loading="refresh" v-ref:scroller>
       <div  v-bind:style="{'min-height':toOCount}">
       <div class="x_report">
  ...
  export default {
    ...
    methods: {
      ...
      getData (obj) {
        ...
        this.$http.post(window.Miu.Data.ApiURL + url, {
          ...
          if (obj.type === 'toMe') {
            if (obj.init) {
              this.toMCount = 180 * res.data.list.Tag.Approvals.length + 'px'
            }
            this.toMe1 = false
            if (obj.method === 'more') {
            ...
            if (obj.init) {
              this.toOCount = 180 * res.data.list.Tag.Approvals.length + 'px'
            }
            this.toOther1 = false
            if (obj.method === 'more') {

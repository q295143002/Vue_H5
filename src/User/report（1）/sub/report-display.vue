   ...
   <x-scroller  v-show="!toMe1" lock-x scrollbar-y use-pullup use-pulldown  height=""  @pullup:loading="loadMore" @pulldown:loading="refresh" v-ref:scroller>
     <div class="x_report">
       <group v-for="items in toMeData" class="animated" v-if="items.ApprovalType==='国内机票'">
         ...
         <cell @click='$router.go("/report/flight/"+items.ApprovalID)'><!--假数据传参-->
        ...
        var obj = {
          ...
          'fn1': function () {
            _self.$nextTick(() => {
              ...
              console.log(1)
              // document.querySelectorAll('.xs-plugin-pullup-up')[0].innerHTML = ''
              // document.querySelectorAll('.xs-plugin-pullup-up')[1].innerHTML = ''
            })
          },
          'method': 'more'
        }
